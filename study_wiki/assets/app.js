(() => {
  ...
  const { filter, terms, unclassified, examKey, isPlaceholder, canonical } = window.WikiSearch;
  ...
  function render() {
    ...
    for (const [key, field] of [['topic', null], ['structure', 'data_structures'], ['algorithm', 'algorithms']]) {
      if (state[key]) state[key] = canonical(state[key], data.taxonomy, field);
    }
