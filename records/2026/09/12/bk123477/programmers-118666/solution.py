...
def solution(survey, choices): # survey: RT/TR, FC/CF, MJ/JM, AN/NA (순서대로 비동의, 동의시) - (사전순 우위 R, C, J, A)
    answer = ''
    mbti_dict = defaultdict(int)
    
    for s, c in zip(survey, choices):
        if c > 4: # 동의
            mbti_dict[s[1]] += abs(c-4)
        else:
            mbti_dict[s[0]] += abs(c-4)
    
    rc, tc = mbti_dict["R"], mbti_dict["T"]
    if rc >= tc:
        answer+="R"
    else:
        answer+="T"
    
    cc, fc = mbti_dict["C"], mbti_dict["F"]
    if cc >= fc:
        answer+="C"
    else:
        answer+="F"
        
    jc, mc = mbti_dict["J"], mbti_dict["M"]
    if jc >= mc:
        answer+="J"
    else:
        answer+="M"
        
    ac, nc = mbti_dict["A"], mbti_dict["N"]
    if ac >= nc:
        answer+="A"
    else:
        answer+="N"
