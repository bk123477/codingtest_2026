...
def solution(board, moves): # 15351214
    answer = 0
    N = len(board)
    game = [[] for _ in range(N)] # transpose
    for i in range(N):
        for j in range(N):
            game[j].append(board[i][j])
    stack = []
    for move in moves:
        for i in range(N):
            if game[move-1][i] == 0:
                ...
                if len(stack) > 0 and stack[-1] == game[move-1][i]:
                    answer+=2
                    stack.pop()
                else:
                    stack.append(game[move-1][i])
                game[move-1][i] = 0
                break
