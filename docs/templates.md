...
코딩 에이전트에게 이 과정을 맡길 수도 있습니다. 저장소 루트의 [AGENTS.md](../AGENTS.md)를 먼저 읽게 한 뒤, 문제 링크·풀이 파일 또는 학습 메모를 제공하세요. 에이전트가 같은 CLI를 실행하고 생성된 README를 정리합니다. `solution.py`만 작성한 경우에도 코드·기존 README·`meta.json`을 읽어 분류를 보완할 수 있으며, 기존 taxonomy에 없는 핵심 자료구조·알고리즘은 별도 taxonomy 관리 PR에서 `--add-taxonomy`로 제안합니다. 복사해서 사용할 요청 예시는 [README의 에이전트 안내](../README.md#코딩-에이전트로-기록하기)에 있습니다.
...
분류·태그는 동의어와 띄어쓰기를 정규화합니다. 새로운 자료구조·알고리즘은
`enrich.py --add-taxonomy`로 taxonomy에 추가하며, 자세한 기준은
[Wiki 분류 안내](wiki.md#핵심-분류와-세부-검색어)를 참고하세요.
...
LLM이 새 자료구조·알고리즘을 제안한 경우 `new` 또는 `done`에 `--add-taxonomy`를 붙이면
표준명으로 taxonomy에 등록합니다.
