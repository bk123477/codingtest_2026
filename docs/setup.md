...
문제와 정리는 같은 목표에 합산합니다. 목표 없이 참여시키려면 JSON의 `daily_goal`을 `null`로 두거나, 참여자에게 `python3 study.py init 아이디 --goal none`을 안내하세요. `null` 목표는 현황에서 자율 기록으로 표시되고 미달로 판단하지 않습니다.
...
목표 미달은 PR 실패 사유가 아닙니다. 장기간 미달이 반복되는지 확인하고 팀 규칙이나 개인 목표를 조정하는 용도로 사용하세요. 자율 기록(`daily_goal: null`)은 완료 건수만 보이므로 달성률 관리 대상에서 제외됩니다.
