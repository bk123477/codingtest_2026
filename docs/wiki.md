...
풀이자가 `solution.py`만 작성했다면 문제 기록 폴더와 URL을 함께 알려 주세요. 에이전트는 코드를 읽어 README와 `meta.json`을 보완하고, `study_wiki/taxonomy.json`의 표준명·별칭을 먼저 대조합니다. 기존 개념이면 표준명을 사용하고, 코드에서 확인된 새로운 핵심 자료구조·알고리즘이면 `--add-taxonomy`를 사용해 taxonomy 관리 PR에서 표준명과 별칭을 검토합니다. Python·라이브러리·구현 세부 같은 일반 태그는 taxonomy가 아니라 검색어로만 남깁니다.
...
LLM이 새 자료구조·알고리즘을 제안한 경우에만 `--add-taxonomy`를 추가하세요.
이 옵션은 입력한 자료구조·알고리즘 중 기존 taxonomy에 없는 표준명을 `taxonomy.json`에
등록하고, 기록 메타데이터에도 같은 표준명을 저장합니다. `--tags`의 일반 태그는 등록하지 않습니다.
새 기록의 `new`와 완료 처리의 `done`에도 같은 옵션을 사용할 수 있습니다.

분류 별칭은 `study_wiki/taxonomy.json`에 관리합니다. 기존 기록 파일은 build가 수정하지 않습니다.
LLM이나 코딩 에이전트가 실제 코드에서 기존 자료구조·알고리즘을 확인하면 taxonomy의 표준명을
사용합니다. 정말 새로운 자료구조·알고리즘이면 표준명과 확인된 별칭을 taxonomy에 함께 제안하고,
검토 가능한 taxonomy 관리 PR에서 등록합니다. 모든 반복문을 완전 탐색으로, 모든 리스트를 스택으로
분류하지 마세요.

일일 기록을 작성하는 중 새 핵심 개념을 발견하면 일일 기록 PR에 `taxonomy.json`을 섞지 않습니다.
관리 브랜치에서 `--add-taxonomy`로 등록 후보를 반영하고 표준명·별칭 충돌과 코드 근거를 검토한 뒤,
taxonomy가 main에 병합되면 일일 기록의 분류를 같은 표준명으로 동기화합니다.
...
Wiki의 필터·개념 연결은 `study_wiki/taxonomy.json`에 정의한 핵심 자료구조와
알고리즘만 사용합니다. 배열·해시·스택·큐·힙·트리·그래프 및
구현·문자열 처리·수학·정렬·완전 탐색·이진 탐색·DFS/BFS·DP·그리디 등이 기준입니다.
기록에 없는 분류는 목록에 나타나지 않습니다. 같은 개념을 자료구조·알고리즘·태그에
중복 입력해도 Wiki에서는 한 번만 표시하고, 올바른 분류 필터로 연결합니다.

`new`, `note`, `done`의 분류 입력과 `enrich.py`는 같은 정규화 함수를 사용합니다.
`완전탐색` / `완전 탐색` / `brute force`는 `완전 탐색`으로,
`우선순위큐` / `priority queue`는 `힙`으로 저장합니다.
Unicode 호환 문자·영문 대소문자·공백(탭, 전각 공백 포함)은 분류를 나누지 않습니다.
등록된 표준명 안의 띄어쓰기는 화면에 유지합니다.
`순회`, `반복문` → `구현`, `나머지 연산` → `수학`처럼 세부 기법을 상위 분류로
묶는 규칙도 포함합니다. 이는 세부 기법이 모두 동의어라는 뜻은 아닙니다.

기록을 분류할 때는 먼저 기존 taxonomy의 표준명·별칭으로 통합합니다. 예를 들어
`완전탐색` / `완전 탐색` / `brute force`는 `완전 탐색`으로, `우선순위큐` / `priority queue`는
`힙`으로 기록합니다. 띄어쓰기·대소문자·Unicode 호환 문자 차이는 분류를 나누지 않습니다.

기존 taxonomy에 없는 표현이 실제로 새로운 자료구조·알고리즘이라면 그대로 버리지 않습니다.
LLM 출력은 다음처럼 `new_taxonomy`에 종류별 표준명과 별칭을 함께 제안해야 합니다.

```json
{
  "topics": ["세그먼트 트리"],
  "new_taxonomy": {
    "data_structures": [{"name": "세그먼트 트리", "aliases": ["segment tree"]}],
    "algorithms": []
  }
}
```

AI 개념 노트 생성은 이 제안을 검증한 뒤 `taxonomy.json`과 노트가 함께 검토되는 관리 PR을
만듭니다. 기록을 보완하는 에이전트도 taxonomy 관리 브랜치에서 `enrich.py`에
`--add-taxonomy`를 붙여 같은 동작을 사용할 수 있습니다.
...
새 항목은 실제 풀이에서 사용한 근거가 있어야 하며, 표준명·별칭 충돌은 저장 전에 거부합니다.
taxonomy 변경은 일반 코드·기록과 마찬가지로 관리 PR에서 사람이 확인합니다. 일일 기록 PR에는
taxonomy 파일을 섞지 않습니다. 언어·도구·개인
메모처럼 자료구조·알고리즘이 아닌 태그는 자유 검색어로 보존하며 taxonomy에 추가하지 않습니다.
