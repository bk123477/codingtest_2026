...
**처음 보는 표현은 의미를 추측해 자동 분류하지 않습니다.** 원본 메타데이터와 검색에
보존하고 상세 화면의 ‘세부 검색어’에 표시하지만, 새로운 필터나 개념 노드를 만들지는
않습니다. 따라서 미래의 모든 표현을 미리 나열하지 않아도 분류가 계속 쪼개지지 않습니다.
알려지지 않은 동의어까지 자동으로 의미를 판별하는 기능은 아닙니다.
새 표현에도 개념 연결이 필요하면 기존 핵심 분류를 함께 입력하세요.
예: `--tags "새로운 세부 기법, 동적 계획법"`.
반복해서 쓰는 별칭이나 꼭 필요한 새 핵심 개념은 taxonomy 변경 PR에서 검토하여 추가합니다.
서로 다른 표준명에 같은 별칭을 등록하면 검증에서 충돌을 발견합니다.

기존 기록은 빌드할 때 동일한 규칙으로 변환하므로 원본 README·meta·완료 상태를
일괄 수정할 필요가 없습니다. 세부 원래 표현도 검색할 수 있습니다.
이전 개념 링크의 `완전탐색` 같은 별칭도 표준 개념으로 연결됩니다.
언어·도구·개인 메모 등 핵심 분류 밖의 태그는 검색어로 사용할 수 있습니다.
