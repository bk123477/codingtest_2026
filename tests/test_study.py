...
class StudyTest(unittest.TestCase):
    ...
    def test_goal_none_allows_mixed_self_directed_records(self):
        ...
        self.assertIn('2건 (자율)', body)
        self.assertIn('2건 / 자율', body)
        self.assertNotIn('목표 미달', self.cli('prepare', '--date', DAY))
        daily = (study.daily_path('alice', DAY) / 'README.md').read_text(encoding='utf-8')
        self.assertIn('완료 **2건** · 자율 기록', daily)
        report = study.dashboard(DAY)
        self.assertIn('| alice | 2건 / 자율 | 2 |', report)
        self.assertIn('자율 2건', report)
        self.cli('goal', '3', '--from', '2026-09-06')
        self.assertEqual(study.goal_at(study.profiles()['alice'], '2026-09-06'), 3)
        self.cli('goal', 'none', '--from', '2026-09-07')
        self.assertIsNone(study.goal_at(study.profiles()['alice'], '2026-09-07'))
        self.cli('check')
