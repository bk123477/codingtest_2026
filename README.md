...
### `solution.py`만 작성한 경우

문제 기록을 먼저 만든 뒤 `solution.py`만 작성해도 됩니다. 에이전트에게 기록 폴더와 문제 URL을 알려 주면 `solution.py`·기존 README·`meta.json`을 읽어 실제 코드에 맞는 문제 요약, 풀이 설명, 복잡도와 분류를 보완합니다. 사용자가 작성한 코드와 회고는 보존하고, 원문이나 실행 확인이 없는 내용은 추측하지 않습니다.

분류는 에이전트가 먼저 [study_wiki/taxonomy.json](study_wiki/taxonomy.json)을 확인합니다. 이미 있는 자료구조·알고리즘은 표준명으로 기록하므로 `완전탐색`, `완전 탐색`, `brute force`처럼 표현이 달라도 하나로 연결됩니다. 코드에서 실제로 사용한 새로운 핵심 자료구조·알고리즘을 발견하면 `--add-taxonomy`를 사용해 표준명을 taxonomy에 제안하고, 확인한 동의어 별칭은 taxonomy 관리 PR에서 함께 검토합니다. Python·라이브러리·구현 세부 같은 일반 태그는 taxonomy를 늘리지 않고 검색어로 보존합니다.

에이전트에게는 다음처럼 요청할 수 있습니다.

```text
AGENTS.md를 읽고 records/YYYY/MM/DD/사용자명/programmers-문제번호/ 기록을 보완해줘.
내가 작성한 solution.py와 기존 README를 읽고 실제 코드에 근거해 README와 meta.json을 채워줘.
분류 전에 study_wiki/taxonomy.json을 확인하고, 기존 개념이면 표준명을 사용해줘.
정말 새로운 핵심 자료구조·알고리즘이면 taxonomy 관리 PR에서 --add-taxonomy로 등록할 후보와 별칭을 제안해줘.
코드·회고·채점 결과·실행 결과는 추측하거나 바꾸지 말고, 검증 결과와 남은 TODO를 알려줘.
```
